...
def graph_chute_par_clause(pdf_clauses, d: dict):
    ...
    _title(
        ...
        f"Top {len(pdf)} clauses par PM MRM — base chute : tous les dossiers retrouvés "
        f"au compte (inventaire + récupérés N+1)",
    )
...
def graph_chute_par_consigne(d: dict):
    ...
    _title(
        ...
        "Positif = sous-provisionné (risque) — retrouvés (inventaire + N+1) ; le global "
        "inclut aussi « à supprimer » retrouvées et sans-consigne, suivies à part",
    )
...
def graph_kpi_chute_globale(d: dict):
    ...
    _title(
        ...
        "Σ(PM MRM − PM CPT) / Σ PM MRM — base chute : tous les dossiers retrouvés "
        "au compte (inventaire + récupérés N+1)",
    )
...
def graph_pm_par_consigne(d: dict):
    ...
    _title(
        ...
        "Δ = PM MRM − PM compte (positif = sous-provisionné) — retrouvés (inventaire + N+1) ; "
        "le global inclut aussi « à supprimer » retrouvées et sans-consigne",
    )
