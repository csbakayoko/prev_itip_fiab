...
def graph_chute_par_clause(pdf_clauses, d: dict):
    ...
    _title(
        ...
        f"Top {len(pdf)} clauses par PM MRM — base chute : retrouvés (inventaire + N+1) "
        f"hors « à supprimer » et statut inventaire NON",
    )
...
def graph_kpi_chute_globale(d: dict):
    ...
    _title(
        ...
        "Σ(PM MRM − PM CPT) / Σ PM MRM — base chute : retrouvés (inventaire + N+1) "
        "hors « à supprimer » et statut inventaire NON",
    )
